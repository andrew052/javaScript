...
function searchStudent() {
  ...
  for (let i = 0; i < students.length; i++) {
    if (students[i].numberDocument === searchDocument) {
      console.log(`Estudiante encontrado: ${students[i].name} ${students[i].lastName}, Documento: ${students[i].typeDocument} ${students[i].numberDocument}`);
      found = true;
      break;
    }
  }
...
function modifyStudent() {
  ...
  for (let i = 0; i < students.length; i++) {
    if (students[i].numberDocument === searchDocument) {
      let option = prompt("¿Qué desea modificar? 1-Nombre");
      switch (option) {
        case "1":
...
function ordenarAlfabeticamente(students) {
